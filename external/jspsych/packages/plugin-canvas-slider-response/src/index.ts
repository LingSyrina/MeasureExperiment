...
class CanvasSliderResponsePlugin implements JsPsychPlugin<Info> {
  ...
  trial(display_element: HTMLElement, trial: TrialType<Info>) {
    ...
    html +=
      ...
      'step="' + trial.step + '" ' +
      'style="width: 100%;" ' +
      'id="jspsych-canvas-slider-response-response" ' +
      'data-started="false">';
    html += "<div>";
    ...
    const sliderEl = display_element.querySelector<HTMLInputElement>("#jspsych-canvas-slider-response-response");
    if (sliderEl) {
      sliderEl.value = trial.min.toString();  // instead of ""
      sliderEl.setAttribute("data-started", "false");
      sliderEl.classList.add("inactive-slider");  // 💥 add this class
    }
    ...
    if (trial.require_movement) {
      const slider = display_element.querySelector<HTMLInputElement>("#jspsych-canvas-slider-response-response");
      const button = display_element.querySelector<HTMLButtonElement>("#jspsych-canvas-slider-response-next");

      button.disabled = true;

      const enable_button = () => {
        if (slider.getAttribute("data-started") === "false") {
          slider.setAttribute("data-started", "true");
          button.disabled = false;
          slider.classList.remove("inactive-slider");  // 💥 remove the visual "unset" look
        }
      };


      slider.addEventListener("input", enable_button);
    }
