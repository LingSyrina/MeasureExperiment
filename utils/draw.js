function drawBlob(ctx, blob, offsetX, offsetY, config) {
...
function getAllPoints(blob) {
  return blob.x.map((x, i) => ({ x, y: blob.y[i], type: 'anchor', index: i }))
    .concat(blob.ctrlX1.map((x, i) => ({ x, y: blob.ctrlY1[i], type: 'control', index: i })));
}

function interpolate2D(A, B, C, D, tx, ty) {
  const AB = MorphBlob.interpolate(A, B, tx); // top edge
  const CD = MorphBlob.interpolate(C, D, tx); // bottom edge
  return MorphBlob.interpolate(AB, CD, ty);   // vertical between top and bottom
}
...
function drawDraggablePoints(state, blob, key) {
